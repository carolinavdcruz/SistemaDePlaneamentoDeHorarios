...
No backend, a API foi implementada em Ktor, com persistência em PostgreSQL.
...
O professor pode gerir a sua disponibilidade, definir restrições, consultar os alunos associados, criar propostas de horário, confirmar essas propostas, consultar aulas já persistidas, marcar presenças, cancelar ou remarcar sessões e enviar notificações aos alunos. O aluno pode escolher um professor, registar a sua disponibilidade, definir a sua carga horária semanal pretendida, consultar as aulas que lhe estão atribuídas e acompanhar o respetivo estado.
...
\begin{itemize}
    ...
    \item geração e persistência de aulas concretas com recorrência;
    \item consulta de aulas semanais e histórico;
...
Esta solução revelou-se adequada para suportar o conjunto de operações necessárias e para validar a integração com a aplicação móvel.
...
\begin{itemize}
    ...
    \item criação de propostas de horário;
    \item geração de aulas concretas com datas reais;
    \item consulta semanal e histórica de aulas;
    \item marcação de presenças;
    \item cancelamento e remarcação de aulas;
    \item envio de notificações e integração com Google Calendar.
\end{itemize}
...
e o backend colaboram para concretizar os fluxos principais do projeto.
...
nomeadamente o reforço da sincronização entre dados locais e remotos, a evolução da componente de disponibilidade e a melhoria da apresentação dos horários.
Ainda assim, a base estrutural da solução encontra-se definida e suficientemente sólida para suportar a utilização do sistema no contexto previsto e a sua evolução futura com novas funcionalidades.