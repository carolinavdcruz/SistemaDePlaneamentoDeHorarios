...
posteriormente para a geração de propostas de horário. No caso das restrições, os dados configurados pelo professor são também persistidos, 
...
\section{Geração de Horários no Backend}

Uma das evoluções mais relevantes da versão beta foi a passagem da lógica de geração de horários para o backend. 
...
\begin{itemize}
    ...
    \item gera sessões compatíveis com os dados disponíveis.
\end{itemize}
...
A aplicação Android passou a consumir a API para operações relacionadas com utilizadores, disponibilidades, restrições e, mais recentemente, geração de horários.
